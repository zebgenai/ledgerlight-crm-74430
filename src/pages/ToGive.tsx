...
export default function ToGive() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      toGiveSchema.parse({ ...formData, amount: Number(formData.amount) });
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    const { error } = await supabase.from("to_give").delete().eq("id", id);
  ...
  const startEdit = (record: ToGiveRecord) => {
    setFormData({ 
  ...
  const handleExport = (format: 'csv' | 'excel') => {
    ...
    const filename = `to-give-${new Date().toISOString().split('T')[0]}`;
    
    if (format === 'csv') {
      exportToCSV(exportData, filename);
    } else {
      exportToExcel(exportData, filename, 'To Give');
    }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) resetForm(); }}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editingId ? "Edit" : "Add"} To Give</DialogTitle>
